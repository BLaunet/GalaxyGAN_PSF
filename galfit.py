...
def create_feedme_PSF(feedmefile, ifile, resultfile, ipath, platescale, centroid, imshape):
    ...
    file_res.write('H)  '+ str(xmin)+' '+ str(xmax)+' '+str(ymin)+' '+str(ymax)+'  # Image region to fit (xmin xmax ymin ymax) \n')
...
def fit_PSF_GALFIT(fname, out_dir):
    '''
    :param fname: filename of PSF image, the image must be background subtracted!!!
    :param fpath: path to directory of PSF image
    :param maxrad: maximal radius from the centroid before the noise cutoff
    :param centroid: centroid coordinates [x_centroid, y_centroid]
    :return: PSF triple gaussian model
    '''
    print(fname)
    hdu = fits.open(fname)
