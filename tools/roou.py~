...
def roou():
    ...
    for i in files:
        ...
        #centroid_coord = find_centroid(data_r, np.array([center_coord]), 5) had to deactivate because photutils is not installed
        ...
        if mode:
            jpg_path = '%s/test/%s.jpg'% (figure,image_id)
        else:
            jpg_path = '%s/train/%s.jpg'% (figure,image_id)

        if mode == 2:
            figure_combined_no_ori = np.zeros(figure_original.shape[0], figure_original.shape[1]*2,1)
            figure_combined_no_ori[:, figure_original.shape[1]:2*figure_original.shape[1],:] = figure_with_PSF[:,:,:]
