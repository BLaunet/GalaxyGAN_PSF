...
def SExtractor_get_stars(path, filename, magzero, threshold, saturation_level, gain, pixel_scale, fwhm, imageshape, edge, mindist):
...
def get_field(obj_line):
    ...
    if not os.path.isfile('%s%s/sdss%s_dr12_%s-%s.fits.bz2' % (core_path, relative_path, filter_string, run, field)):
        os.system('cd %s; rsync --relative -avzh dostark@plompy.ethz.ch:/home/galaxian/source/sdss/dr12/plates/./%s/sdss%s_dr12_%s-%s.fits.bz2 .' % (core_path, relative_path, filter_string, run, field))
...
def add_sdss_PSF(origpath, original, psf_flux, obj_line, whitenoise_var = None, multiple=False, sexdir = None, median_combine=False):
    ...
    if sexdir:
        ...
        sex_edge = 46
        x_coordinates, y_coordinates, fluxes, starboolean = SExtractor_get_stars(sexdir, 'field_ADU.fits', zeropoint, threshold, saturation_limit, gain, pixel_scale, fwhm, field_data.shape, sex_edge, mindist = 30)
        if not starboolean:
        ...
        if median_combine:
            ...
            psf_unscaled = empirical_PSF_manual(field_data, x_crds[fluxmask], y_crds[fluxmask], csize, fwhm/pixel_scale, obj_id)
...
def rebin_undersample(data, factor):
    ...
    new_x = data.shape[1]/int_factor
    new_y = data.shape[0]/int_factor
    new_data = np.zeros((new_y, new_x))
    for i in range(0, new_x):
        for j in range(0, new_y):
...
def empirical_PSF_manual(data, x_icords, y_icords, cutout_size, fwhm_pix, objid, starplotting=False, plotting_path='no given directory as plot_path'):
    ...
    for j in range(0, len(x_icords)):
        ...
        S, N2, local_bkg = starphot(cutouts[-1], [cutout_size_tmp, cutout_size_tmp], fwhm_pix*3, r_in=fwhm_pix*3, r_out=cutout_size_tmp-1)
    ...
    for image in cutouts:
        ...
        star_centroid = [x_icords[i]-int(x_icords[i])+cutout_size_tmp, y_icords[i]-int(y_icords[i])+cutout_size_tmp]
        ...
        if star_centroid[0] > cutout_size and tmp.shape[1] - cutout_size > star_centroid[0] and star_centroid[
            ...
            cutouts_rebinned.append(tmp[int(round(star_centroid[1])) - cutout_size:int(round(star_centroid[1])) + cutout_size+10,
